...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d edge1 = _x - m_center;
		Eigen::Vector3d edge_dir = Eigen::Vector3d(edge1(0),edge1(1),0);
		Eigen::Vector3d edge2 = edge_dir/edge_dir.norm()*m_radius;
		double length = (edge1 - edge2).norm() - m_a;
		return length;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		VectorXd lhs(m_numCenters+4);
		for (unsigned int i = 0; i < m_numCenters; i++)
		{
			lhs(i) = EvalBasis((m_funcSamp.m_pos[i]-_x).norm());
		}
		lhs(m_numCenters) = _x(0);
		lhs(m_numCenters+1) = _x(1);
		lhs(m_numCenters+2) = _x(2);
		lhs(m_numCenters+3) = 1;
		double result = lhs.transpose()*m_coefficents;
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for (unsigned int j = m_numCenters-1; j < 2*m_numCenters; j++)
		{	
			b(j) = m_funcSamp.m_val[j];
	...
	void SolveSystem()
	{
		...
		//std::cout << m_coefficents << std::endl;
